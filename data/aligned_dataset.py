...
class AlignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if "HER2match" in opt.dataroot:
            self.dir_A = os.path.join(opt.dataroot, 'HE',  opt.phase)  # 创建路径 '/path/to/data/HE/train'
            self.dir_B = os.path.join(opt.dataroot, 'IHC', opt.phase)  # 创建路径 '/path/to/data/IHC/train'
            ...
            if opt.phase == "test" and not os.path.exists(self.dir_A) \
                ...
                self.dir_A = os.path.join(opt.dataroot, "valA_full")
                self.dir_B = os.path.join(opt.dataroot, "valB_full")
            if opt.phase == "temp" and not os.path.exists(self.dir_A) \
                    and os.path.exists(os.path.join(opt.dataroot, "tempA")):
                self.dir_A = os.path.join(opt.dataroot, "tempA")
                self.dir_B = os.path.join(opt.dataroot, "tempB")
    ...
    def __getitem__(self, index):
        ...
        if "HER2match" in self.opt.dataroot:
            assert A_path == B_path.replace('IHC', 'HE')
        else:
            assert A_path == B_path.replace('trainB', 'trainA').replace('valB', 'valA').replace('testB', 'testA').replace('tempB', 'tempA')
